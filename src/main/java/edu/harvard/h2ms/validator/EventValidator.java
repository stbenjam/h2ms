        ...
        if (event.getEventTemplate() == null) {
            ...
            for (Answer answer : event.getAnswers()) {
                ...
                if (!questions.contains(answer.getQuestion())) {
                    errors.rejectValue(
                            String.format("answers[%d].value", idx), "Answer.NotInTemplate");
                }
                ...
                switch (answer.getAnswerType()) {
                    case "options":
                        if (!answer.getQuestion().getOptions().contains(answer.getValue())) {
                            errors.rejectValue(
                                    String.format("answers[%d].value", idx), "Answer.NotInOptions");
                        }
                        break;
                    case "boolean":
                        if (!asList("true", "false").contains(answer.getValue())) {
                            errors.rejectValue(
                                    String.format("answers[%d].value", idx), "Answer.NotBoolean");
                        }
                        break;
                }
